...
| `CEREBRAS_API`, `CEREBRAS_API2`…`CEREBRAS_API16` | Cerebras — llama ultra-rápido | LLM bulk BEEs | ✅ 15 claves |
...
| `FAL_API_KEY` | fal.ai — FLUX.2 + LTX-2 video | Imagen + video | ✅ |
...
- **50 RPM por clave × 15 claves = 750 RPM garantizados**
- Sin límite diario — ideal para BEEs 24/7
- Variables aceptadas (todas): `CEREBRAS_API`, `CEREBRAS_API2`…`CEREBRAS_API16`, `CEREBRA_API5`…`CEREBRA_API14`
...
```
Con BFL_API_KEY (máxima calidad):
  BFL / FLUX.2-dev 32B        → DISPONIBLE #0 ✅

Stable Horde (sin clave):
  SD multi-modelo NSFW        → DISPONIBLE #1 ✅

Con FAL_API_KEY:
  fal.ai / FLUX.2-dev         → DISPONIBLE #2 ✅
  fal.ai / FLUX.2-klein       → DISPONIBLE #3 ✅

Con TOG_API1-5 (tgp_v1_):
  Together / FLUX.1-schnell   → DISPONIBLE #4 ✅
